...
/// @title VeStreet (Street Protocol Voting Escrow)
/// @notice veNFT implementation that escrows STREET tokens in the form of an ERC-721 NFT (veSTREET)
/// @notice Votes have a weight depending on time, so that users are committed to the future of (whatever they are voting for)
/// @author Modified from Solidly / Aerodrome (Street Protocol fork)
/// @dev Vote weight decays linearly over time. Lock time cannot be more than `MAXTIME` (2 years).
contract VeStreet is IVotingEscrow, ERC2771Context, ReentrancyGuard {
    ...
    /// @param _token STREET token address
    ...
    uint256 internal constant MAXTIME = 2 * 365 * 86400; // 2 years (Street Protocol)
