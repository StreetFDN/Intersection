...
contract RewardsDistributorTest is BaseTest {
    ...
    function testInitialize() public {
        ...
        assertEq(distributor.token(), address(STREET));
    ...
    function testClaimWithExpiredNFT() public {
        // test reward claims to expired NFTs are distributed as unlocked STREET
        // ts: 608402
        STREET.approve(address(escrow), TOKEN_1M);
        ...
        assertEq(post - pre, rebase); // expired rebase distributed as unlocked STREET
    ...
    function testClaimRebaseWithManagedLocks() public {
        ...
        assertLt(STREET.balanceOf(address(distributor)), 100); // dust
    }

    function testClaimRebaseWithDepositManaged() public {
        minter.updatePeriod(); // does nothing
        vm.startPrank(address(owner2));
        STREET.approve(address(escrow), TOKEN_10M);
    ...
    function testClaimOnLockedEnd() public {
        ...
        // Rebase should deposit into veNFT one second before expiry
        uint256 balanceBefore = STREET.balanceOf(address(owner));
        distributor.claim(tokenId);
        assertGt(STREET.balanceOf(address(owner)), balanceBefore);
    }
}
