...
contract LockedManagedRewardTest is BaseTest {
    ...
    function testCannotNotifyRewardIfNotVotingEscrow() public {
        ...
        lockedManagedReward.notifyRewardAmount(address(STREET), 0);
    }
    ...
    function testNotifyRewardAmount() public {
        deal(address(STREET), address(escrow), TOKEN_1 * 3);

        vm.prank(address(escrow));
        STREET.approve(address(lockedManagedReward), TOKEN_1);
        uint256 pre = STREET.balanceOf(address(escrow));
        vm.prank(address(escrow));
        vm.expectEmit(true, true, true, true, address(lockedManagedReward));
        emit NotifyReward(address(escrow), address(STREET), 604800, TOKEN_1);
        lockedManagedReward.notifyRewardAmount(address(STREET), TOKEN_1);
        uint256 post = STREET.balanceOf(address(escrow));

        assertEq(lockedManagedReward.isReward(address(STREET)), true);
        ...
        vm.prank(address(escrow));
        STREET.approve(address(lockedManagedReward), TOKEN_1 * 2);
        pre = STREET.balanceOf(address(escrow));
        vm.prank(address(escrow));
        vm.expectEmit(true, true, true, true, address(lockedManagedReward));
        emit NotifyReward(address(escrow), address(STREET), 604800, TOKEN_1 * 2);
        lockedManagedReward.notifyRewardAmount(address(STREET), TOKEN_1 * 2);
        post = STREET.balanceOf(address(escrow));

        assertEq(lockedManagedReward.tokenRewardsPerEpoch(address(STREET), 604800), TOKEN_1 * 3);
        assertEq(pre - post, TOKEN_1 * 2);
        assertEq(STREET.balanceOf(address(lockedManagedReward)), TOKEN_1 * 3);
    }
    ...
    function _addLockedReward(uint256 _amount) internal {
        deal(address(STREET), address(distributor), _amount);
        vm.startPrank(address(distributor));
        STREET.approve(address(escrow), _amount);
