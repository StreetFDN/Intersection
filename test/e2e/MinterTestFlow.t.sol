...
contract MinterTestFlow is ExtendedBaseTest {
    ...
    function testMinterRebaseFlow() public {
        /// epoch 0
        minter.updatePeriod();
        assertEq(STREET.balanceOf(address(voter)), 0);

        STREET.approve(address(escrow), TOKEN_100K);
        ...
        minter.updatePeriod();
        assertEq(STREET.balanceOf(address(voter)), expectedMint);

        address[] memory gauges = new address[](1);
        gauges[0] = address(gauge);

        uint256 epochStart = _getEpochStart(block.timestamp);
        assertEq(STREET.allowance(address(voter), address(gauge)), 0);
        voter.distribute(gauges);
        assertEq(STREET.allowance(address(voter), address(gauge)), 0);
        assertApproxEqRel(STREET.balanceOf(address(gauge)), expectedMint / 2, 1e6);
        assertApproxEqRel(STREET.balanceOf(address(voter)), expectedMint / 2, 1e6);
        ...
        voter.distribute(gauges);
        assertApproxEqRel(STREET.balanceOf(address(gauge2)), expectedMint / 2, 1e6);
        assertLt(STREET.balanceOf(address(voter)), 1e6); // dust
        skipAndRoll(1);
        ...
        /// epoch 2
        skipToNextEpoch(1);
        uint256 balance = STREET.balanceOf(address(gauge));
        ...
        assertLt(STREET.balanceOf(address(voter)), 1e6);
        assertApproxEqRel(STREET.balanceOf(address(gauge)), balance, 1e6);
        assertApproxEqRel(STREET.balanceOf(address(gauge)), balance, 1e6);
        ...
        assertApproxEqAbs(STREET.balanceOf(address(voter)), 8_744_211 * TOKEN_1, TOKEN_1);
        voter.distribute(0, voter.length());
        ...
        assertApproxEqAbs(STREET.balanceOf(address(voter)), 8_968_681 * TOKEN_1, TOKEN_1);
        voter.distribute(0, voter.length());
        ...
        assertApproxEqAbs(STREET.balanceOf(address(voter)), 9_064_968 * TOKEN_1, TOKEN_1);
        voter.distribute(0, voter.length());
        ...
        assertApproxEqAbs(STREET.balanceOf(address(voter)), 9_027_516 * TOKEN_1, TOKEN_1);
        voter.distribute(0, voter.length());
    }
