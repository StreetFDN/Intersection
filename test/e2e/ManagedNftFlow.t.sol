...
contract ManagedNftFlow is ExtendedBaseTest {
    ...
    function testSimpleManagedNftFlow() public {
        ...
        rewards[0] = address(STREET);
        rewards[1] = address(USDC);

        // create aero bribe for next epoch
        _createBribeWithAmount(bribeVotingReward, address(STREET), TOKEN_1 * 2);
        ...
        assertEq(escrow.supply(), supply);
        assertEq(STREET.balanceOf(address(lockedManagedReward)), TOKEN_1);
        assertEq(lockedManagedReward.earned(address(STREET), tokenId), 0);
        ...
        assertEq(lockedManagedReward.earned(address(STREET), tokenId), TOKEN_1 / 2);
        assertEq(lockedManagedReward.earned(address(STREET), tokenId2), TOKEN_1 / 2);
        ...
        uint256 pre = STREET.balanceOf(address(owner4));
        vm.prank(address(voter));
        bribeVotingReward.getReward(mTokenId, rewards);
        uint256 post = STREET.balanceOf(address(owner4));
        ...
        STREET.approve(address(freeManagedReward), TOKEN_1 * 2);
        freeManagedReward.notifyRewardAmount(address(STREET), ((TOKEN_1 * 2) * 3) / 4);
        vm.stopPrank();
        ...
        assertEq(lockedManagedReward.earned(address(STREET), tokenId), TOKEN_1 / 2);
        assertEq(freeManagedReward.earned(address(STREET), tokenId), 0);
        assertEq(lockedManagedReward.earned(address(STREET), tokenId2), TOKEN_1 / 2);
        assertEq(freeManagedReward.earned(address(STREET), tokenId2), 0);
        ...
        assertEq(lockedManagedReward.earned(address(STREET), tokenId), TOKEN_1);
        assertEq(freeManagedReward.earned(address(STREET), tokenId), ((TOKEN_1 * 2) * 3) / 4 / 2);
        assertEq(lockedManagedReward.earned(address(STREET), tokenId2), TOKEN_1);
        assertEq(freeManagedReward.earned(address(STREET), tokenId2), ((TOKEN_1 * 2) * 3) / 4 / 2);
        ...
        skip(1 hours);
        pre = STREET.balanceOf(address(escrow));
        vm.prank(address(owner2));
        voter.withdrawManaged(tokenId2);
        post = STREET.balanceOf(address(escrow));
        ...
        assertEq(STREET.balanceOf(address(lockedManagedReward)), TOKEN_1 * 2);

        // check nfts are configured correctly
        locked = escrow.locked(tokenId2);
        assertEq(uint256(uint128(locked.amount)), TOKEN_1 * 2);
        assertEq(locked.end, 64713600);
        assertEq(locked.isPermanent, false);
        ...
        assertEq(lockedManagedReward.earned(address(STREET), tokenId2), 0);
        assertEq(freeManagedReward.earned(address(STREET), tokenId2), ((TOKEN_1 * 2) * 3) / 4 / 2);
        ...
        assertEq(lockedManagedReward.earned(address(STREET), tokenId), 0);
        assertEq(STREET.balanceOf(address(lockedManagedReward)), 0);
        ...
        _createBribeWithAmount(bribeVotingReward, address(STREET), TOKEN_1);

        skipToNextEpoch(1);

        // test normal nft behavior post withdrawal
        // ~= approx TOKEN_1 * 3 / 4, some drift due to voting power decay
        assertEq(bribeVotingReward.earned(address(STREET), tokenId), 749095271054930289);
    ...
    function testTransferManagedNftFlow() public {
        ...
        vm.startPrank(address(owner4));
        STREET.approve(address(freeManagedReward), TOKEN_1 * 2);
        freeManagedReward.notifyRewardAmount(address(STREET), TOKEN_1);
        vm.stopPrank();
        ...
        assertEq(freeManagedReward.earned(address(STREET), tokenId), TOKEN_1 / 2);
        assertEq(freeManagedReward.earned(address(STREET), tokenId2), TOKEN_1 / 2);
        ...
        // collect rewards from bribe
        pre = STREET.balanceOf(address(owner3));
        vm.prank(address(voter));
        bribeVotingReward.getReward(mTokenId, rewards);
        post = STREET.balanceOf(address(owner3));
        assertEq(post - pre, TOKEN_1 * 2);

        // distribute reward to managed nft depositors
        vm.startPrank(address(owner3));
        STREET.approve(address(freeManagedReward), TOKEN_1 * 2);
        freeManagedReward.notifyRewardAmount(address(STREET), TOKEN_1 * 2);
        vm.stopPrank();
        ...
        assertEq(freeManagedReward.earned(address(STREET), tokenId), TOKEN_1 / 2);
        assertEq(freeManagedReward.earned(address(STREET), tokenId2), (TOKEN_1 * 5) / 2);
    ...
    function testManagedNftRebaseFlow() public {
        ...
        STREET.approve(address(escrow), TOKEN_1M);
        ...
        assertGt(STREET.balanceOf(address(distributor)), 0);
        ...
        assertEq(convert(globalPoint.bias), 2963013674496024015); // (TOKEN_1 * 3 / MAXTIME) * (63504000 - 1814401)
        ...
        assertEq(STREET.balanceOf(address(lockedManagedReward)), managedRebaseTotal);
        assertEq(lockedManagedReward.earned(address(STREET), tokenId), 0);
        assertEq(escrow.supply(), supply);

        // epoch 3: claim rebases
        skipToNextEpoch(1);
        assertEq(lockedManagedReward.earned(address(STREET), tokenId), managedRebaseTotal);
        minter.updatePeriod();
        ...
        // bias / slope not zero due to tokenIds created in _setUp()
        assertEq(convert(globalPoint.bias), 2948630112853896015); // (TOKEN_1 * 3 / MAXTIME) * (63504000 - 2419201)
        assertEq(convert(globalPoint.slope), 23782343985); // TOKEN_1 * 3 / MAXTIME
        ...
        assertEq(lockedManagedReward.earned(address(STREET), tokenId), managedRebaseTotal - rebase);
        assertEq(escrow.supply(), supply);
        ...
        skip(1 hours);
        uint256 pre = STREET.balanceOf(address(escrow));
        voter.withdrawManaged(tokenId);
        uint256 post = STREET.balanceOf(address(escrow));
